    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:flex-col sm:items-end">
          <span className="text-xl font-bold text-primary">Rs. {order.total.toLocaleString('en-NP')}</span>
      ...
      <div className="px-5 pb-4 flex items-center gap-2">
        <div className="flex -space-x-2">
          {order.items.slice(0, 4).map((item) => (
            <div key={item.id} className="w-9 h-9 rounded-full border-2 border-background bg-muted overflow-hidden shrink-0">
                ...
                <div className="flex items-center gap-2 mb-8 flex-wrap">
                  {(['All', 'Processing', 'Shipped', 'Delivered', 'Cancelled'] as const).map((s) => (
                    ...
                    >
                      ...
                      {s === 'All' && <span className="ml-1.5 opacity-70">({orders.length})</span>}
                    </button>
                  ))}
                </div>
